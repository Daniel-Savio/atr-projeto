  ...
  return (
    <div className="flex flex-col justify-center items-center p-4">
      <Card className="w-[96vw]  mx-auto">
        <CardHeader>
          <CardTitle>
            <h1 className="text-2xl font-bold mb-1 text-muted-foreground">
              Carregando Comanda...
            </h1>{" "}
            <Spinner />
          </CardTitle>
...
export default function PaymentComanda({ loaderData }: Route.ComponentProps) {
  ...
  async function closeComanda() {
    ...
    if (response.ok) {
      toast.success("Comanda marcada como fechada com sucesso!", {
        position: "top-right",
      });
      navigate("/pagamento");
    }else{
        ...
        <ScrollArea>
          <CardContent className="h-[45vh]">
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <p
                className={`text-sm font-semibold ${order.status === "Closed" ? "text-green-600" : "text-orange-600"}`}
              >
